...
export const PokemonEvolutions: React.FC<{
	...
	const { data: pokemonSpecies, isLoading: fetchingPokemonSpecies } = useQuery(
		["pokemon-species", url],
	...
	const {
		data: evolutionChain,
		isLoading: fetchingEvolutionChain,
		refetch,
	} = useQuery(
		["evolution-chain", url],
	...
	if (!evolutionChain && fetchingEvolutionChain) {
		return (
			<CardWrapper
				header={
					<div className="w-full flex items-center justify-center">
	...
	} else if (!evolutionChain && !fetchingEvolutionChain) {
		return null;
	}
